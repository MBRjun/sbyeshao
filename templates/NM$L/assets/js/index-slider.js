...
$("#pagination").css({
 ...
 $(function(){
    var w=$(".imglet .slide").width();
    $(".imglet").css({
        width: w*$(".imglet .slide").size() ,
        left:0,
        float:'left',
        //position:'relative',
    });
    ...
    var sw = 0; var sumli=$("#pagination li").size()*1;
    $("#slide-left,#slide-right").click(function(){
        sw = $("#pagination li").index($(".active"));
        sw=sw+1;
        if(sw==sumli){sw=0;}
        myShow(sw);
    });
    $("#pagination li").click(function(){
        sw = $("#pagination li").index(this);
        myShow(sw);
    });
    function myShow(i){    
        if(!$("#feature_slider .imglet").is(":animated")){
            $("#pagination li").eq(i).addClass("active").siblings("li").removeClass("active");
            $("#feature_slider .imglet").animate({marginLeft:"-"+(w*i)+"px"});
        }
        //$("#feature_slider .imglet").css({marginLeft:"-"+(w*i)+"px"});
        //$("#feature_slider .imglet a").eq(i).stop(true,true).fadeIn(600).siblings("a").fadeOut(600);       
    }
    //滑入停止动画，滑出开始动画
    $("#feature_slider .slide").hover(function(){
        if(myTime){
...
$(document).ready(function () {
    function d() {
        ...
        setTimeout(function () {
            $("http://subest.5awo.com/templates/123/assets/js/.slide.active img.fade,.slide.active .info").fadeIn(600, "easeInOutQuad", function () {
    ...
    function c() {
        ...
        function g() {
            ...
            $("http://subest.5awo.com/templates/123/assets/js/.slide.previous img.fade,.slide.previous .info").fadeOut(600, "easeInQuad", function () {
                $("http://subest.5awo.com/templates/123/assets/js/.slide.next").removeClass("next").addClass("active").fadeIn(500, "easeInOutQuad", function () {
